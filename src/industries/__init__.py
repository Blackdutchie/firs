# keep these alphabetised
# - for ease of maintaining
# - object category sort order depends on them
industry_module_names = [
    "appliance_factory",
    "arable_farm",
    "assembly_plant",
    "basic_oxygen_furnace",
    "blast_furnace",
    "body_plant",
    "brewery",
    "builders_yard", 
    "bunker_base",
    "carbon_black_plant",
    "cement_plant",
    "chemical_plant",
    "chlor_alkali_plant",
    "cider_mill",
    "clay_pit",
    "coal_mine",
    "coffee_estate",
    "coke_oven",
    "concrete_plant",
    "construction_yard",
    "copper_mine",
    "copper_smelter",
    "cryo_plant",
    "dairy",
    "dairy_farm",
    "diamond_mine",
    "dredging_site",
    "elastomer_products_plant",
    "electric_arc_furnace",
    "engine_plant",
    "facility_advanced_vehicle_pad",
    "facility_ammunition_factory",
    "facility_basic_vehicle_pad",
    "facility_blast_furnace",
    "facility_coal_refinery",
    "facility_engineering_bay",
    "facility_forge",
    "facility_metal_press",
    "facility_oil_reformer",
    "facility_petrochemical_plant",
    "facility_water_pump",
    "factory_war",
    "farm",
    "field_coal",
    "field_components",
    "field_salvage",
    "field_sulphur",
    "fish_farm",
    "fishing_grounds",
    "fishing_harbour",
    "flour_mill",
    "food_processor",
    "forest",
    "fruit_plantation",
    "garage",
    "general_store",
    "glass_works",
    "hardware_store",
    "herding_coop",
    "hotel",
    "integrated_steel_mill",
    "iron_ore_mine",
    "lime_kiln",
    "limestone_mine",
    "liquids_terminal",
    "manganese_mine",
    "mass_production_factory",
    "metal_works",
    "nitrate_mine",
    "oil_rig",
    "oil_wells",
    "oil_wells_war",
    "orchard_piggery",
    "paper_mill",
    "peatlands",
    "petrol_pump",
    "phosphate_mine",
    "pipe_shop",
    "power_systems_factory",
    "plate_mill",
    "port",
    "potash_mine",
    "power_plant",
    "precision_parts_plant",
    "pyrite_mine",
    "pyrite_smelter",
    "quarry",
    "ranch",
    "refinery_components",
    "refinery_salvage",
    "refinery_sulphur",
    "rubber_plantation",
    "sawmill",
    "scrap_yard",
    "seaport",
    "section_and_bar_mill",
    "slag_grinding_plant",
    "soda_ash_mine",
    "steel_forge_and_foundry",
    "stockyard",
    "storage_depot",
    "strip_mill",
    "supply_yard",
    "timber_yard",
    "tracked_machine_factory",
    "trading_post",
    "tube_and_pipe_mill",
    "tyre_plant",
    "vehicle_distributor",
    "vineyard",
    "wharf",
    "wire_rod_mill",
]
